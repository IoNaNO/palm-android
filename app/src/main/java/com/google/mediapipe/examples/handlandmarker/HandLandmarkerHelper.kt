...
class HandLandmarkerHelper(
    ...
    var currentDelegate: Int = DEFAULT_DELEGATE,
    ...
    fun detectImage(image: Bitmap): ResultBundle? {
        ...
        handLandmarker?.detect(mpImage)?.also { landmarkResult ->
            Log.d(TAG, "Hand Landmarker result: $landmarkResult")
    ...
    // Used to create palm ROI for palm print recognition
    fun getROIImage(image: Bitmap, detectionResult: HandLandmarkerResult): Bitmap? {
        // double check if the detection result is valid
        if (detectionResult.landmarks().isEmpty() || detectionResult.handednesses().isEmpty()) return null
        ...
        // key points
        val handLandmarks = detectionResult.landmarks()[0]
        val x5 = handLandmarks[5].x()
        val y5 = handLandmarks[5].y()
        val x17 = handLandmarks[17].x()
        val y17 = handLandmarks[17].y()
        ...
        val px5 = (x5 * w).toInt()
        val py5 = (y5 * h).toInt()
        val px17 = (x17 * w).toInt()
        val py17 = (y17 * h).toInt()
        ...
        // rotation angle
        val handedness = detectionResult.handednesses()[0][0].categoryName()
        val angle = if (handedness.equals("Right", ignoreCase = true)) {
            Math.toDegrees(Math.atan2((py17 - py5).toDouble(), (px17 - px5).toDouble())).toFloat()
        } else {
            Math.toDegrees(Math.atan2((py5 - py17).toDouble(), (px5 - px17).toDouble())).toFloat()
        }

        // length of ROI
        val length = Math.hypot((px17 - px5).toDouble(), (py17 - py5).toDouble()).toInt()

        // rotation matrix
        val matrix = Imgproc.getRotationMatrix2D(Point(cx.toDouble(), cy.toDouble()), angle.toDouble(), 1.0)

        // Perform rotation
        val rotatedImg = Mat()
        Imgproc.warpAffine(grayMat, rotatedImg, matrix, Size(w.toDouble(), h.toDouble()))

        // Calculate ROI
        val startX = cx - length / 2
        val startY = cy
        val roiImg = Mat(rotatedImg, org.opencv.core.Rect(startX, startY, length, length))

        // Resize to normalized size
        val normalizedImg = Mat()
        Imgproc.resize(roiImg, normalizedImg, Size(256.0, 256.0))

        // Convert back to Bitmap
        val resultBitmap = Bitmap.createBitmap(256, 256, Bitmap.Config.ARGB_8888)
        Utils.matToBitmap(normalizedImg, resultBitmap)
        // normalize ROI
        return resultBitmap
    }
    ...
    companion object {
        ...
        const val DEFAULT_DELEGATE= DELEGATE_GPU
